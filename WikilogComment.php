...
class WikilogComment
{
	...
	static function BeforePageDisplay( &$output, &$skin ) {
		...
		if ( $title->isTalkPage() || $wgWikilogCommentsOnItemPage && $title->getNamespace() === NS_BLOG ) {
...
class WikilogCommentFormatter
{
	...
	protected function getCommentToolLinks( $comment ) {
		global $wgUser, $wgRequest, $wgWikilogCommentQuoteTemplateBegin, $wgWikilogCommentQuoteTemplateEnd;
		$tools = array();
			...
			if ( $this->mAllowReplies && $comment->isVisible() ) {
				...
				$tools['quote'] = Xml::tags( 'a',
					array(
						...
						'href' => '#',
						'onclick' => 'return wlCommentQuotation.onclick(this, "' . $wgWikilogCommentQuoteTemplateBegin . '", "' . $wgWikilogCommentQuoteTemplateEnd . '");',
						'data-text' => $this->prepareCommentQuote( $comment->getText() ) . "\n\n"
					),
	...
	protected function prepareCommentQuote( $text ) {
		...
		return $wgWikilogCommentQuoteTemplateBegin . $text . $wgWikilogCommentQuoteTemplateEnd;
	}
}
