...
- start branch `REL1_44` compatible with MediaWiki >=1.44
...
* (wl:issue 11) Added support for {{DISPLAYTITLE:...}}, allows the author to alter the title of the wikilog article.
...
* (wl:issue 2) Improved article summary extraction. Now a "--more--" marker alone in a line breaks the summary (above) from the rest of the article (below). The <summary> tag has precedence over this new marker. If neither is found, the first section is used.
...
* Wikilog article edit page now features a "sign and publish" checkbox that, when selected, automatically signs and publishes the article by adding an appropriate {{wl-publish:...}} parser function call at the end of the text.
