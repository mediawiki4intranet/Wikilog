...
echo "Wikilog database patch: add foreign keys (MySQL only)\n";
...
$r = $dbw->query("SHOW CREATE TABLE `${P}wikilog_wikilogs`");
$r = $dbw->fetchRow($r);
$r = $r[1];
if (!strpos($r, 'wikilog_wikilogs_wlw_page_page_id'))
{
    $dbw->query("DELETE FROM `${P}wikilog_wikilogs` WHERE (SELECT page_id FROM `${P}page` WHERE page_id=wlw_page) IS NULL");
    $dbw->query("ALTER TABLE `${P}wikilog_wikilogs` ADD CONSTRAINT wikilog_wikilogs_wlw_page_page_id FOREIGN KEY (wlw_page) REFERENCES `${P}page` (page_id) ON DELETE CASCADE ON UPDATE CASCADE");
}
